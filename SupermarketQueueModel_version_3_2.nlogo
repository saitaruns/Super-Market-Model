...
; this read read interpleted value out transaction count out of POS data file and return it as arrival rate (lambda function value )  for Non-Homogeneous Poisson Process
  let value 0
  carefully
      [set value  precision ( (ts-get customer-arrival-input time "customer-transaction-count") / 60 ) 2 ]
      [set value  precision ( (ts-get customer-arrival-input time "customer-transaction-count") / 60 ) 2 ]
  report value
end
  ...
  [
    ...
    ask servers[
      if length server-queue > 0 [
        set min-customers-in-queue min(list min-customers-in-queue length server-queue )
      ]
    ]
